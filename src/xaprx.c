...
#define IN_KEYVALUE        5
	...
	for (i=0; i<strlen((const char *)a_buffer); i++) {
		switch (i_state) {
...
/*						printf("XAPLIB Msg: Name=<%s:%s>, Value=<%s>\n",g_xap_msg[g_xap_index].section, 
																		g_xap_msg[g_xap_index].name,
																		g_xap_msg[g_xap_index].value);
*/					}
					g_xap_index++;
...
int xapmsg_getvalue(const char* a_keyname, char* a_keyvalue)
{
	...
	while ((!matched) && (i++<g_xap_index)) {
		if(g_debuglevel>=DEBUG_VERBOSE)
			fprintf(stderr, "comparing <%s> with <%s> for %d\n",g_xap_msg[i-1].name, a_keyname,i);
